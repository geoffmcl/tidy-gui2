...
// Generated by tidyxmlcfg.pl 2016/09/09 10:53:15 UTC, 12:53:15 local
...
void MarkupTab::on_skip_nested()
{
    const char *label = "skip-nested";
    QCheckBox *w = qobject_cast<QCheckBox *>(sender());
    if (w) {
        Bool b = (Bool)w->isChecked();
        setConfigBool( label, b );
...
void MarkupTab::on_strict_tags_attributes()
{
    const char *label = "strict-tags-attributes";
    QCheckBox *w = qobject_cast<QCheckBox *>(sender());
    if (w) {
        Bool b = (Bool)w->isChecked();
        setConfigBool( label, b );
...
void PrintTab::on_escape_scripts()
{
    const char *label = "escape-scripts";
    QCheckBox *w = qobject_cast<QCheckBox *>(sender());
    if (w) {
        Bool b = (Bool)w->isChecked();
        setConfigBool( label, b );
...
static QCheckBox *skip_nested;
static QCheckBox *strict_tags_attributes;
...
static QCheckBox *escape_scripts;
...
static CHKBXS chkbxs[] = {
    ...
    { &skip_nested, "skip-nested", "Boolean", "markup" },
    { &strict_tags_attributes, "strict-tags-attributes", "Boolean", "markup" },
    ...
    : QWidget(parent)
{
    ...
    skip_nested = new QCheckBox("skip-nested (Boolean)");
    if (getConfigBool("skip-nested")) {
        skip_nested->setChecked(true);
    }
    connect(skip_nested,SIGNAL(clicked()),this,SLOT(on_skip_nested()));


    strict_tags_attributes = new QCheckBox("strict-tags-attributes (Boolean)");
    if (getConfigBool("strict-tags-attributes")) {
        strict_tags_attributes->setChecked(true);
    }
    connect(strict_tags_attributes,SIGNAL(clicked()),this,SLOT(on_strict_tags_attributes()));
    ...
    MarkupLayout2->addWidget(skip_nested);
    MarkupLayout2->addWidget(strict_tags_attributes);
    ...
    : QWidget(parent)
{
    ...
    vertical_space = new QCheckBox("vertical-space (AutoBool)");
    vertical_space->setTristate(true);
    i = getConfigABool("vertical-space");
    if (i == 0) {
        vertical_space->setChecked(false);
        // vertical_space->setCheckState(Qt::Unchecked); // 0
    } else if (i == 1) {
        vertical_space->setChecked(true);
        // vertical_space->setCheckState(Qt::Checked); // 2
    } else {
        vertical_space->setCheckState(Qt::PartiallyChecked); // 1
    }
    ...
    escape_scripts = new QCheckBox("escape-scripts (Boolean)");
    if (getConfigBool("escape-scripts")) {
        escape_scripts->setChecked(true);
    }
    connect(escape_scripts,SIGNAL(clicked()),this,SLOT(on_escape_scripts()));
    ...
    PrintLayout->addWidget(escape_scripts);
