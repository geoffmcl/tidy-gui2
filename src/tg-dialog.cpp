...
// Generated by tidyxmlcfg.pl 2015/09/19 17:43:27 UTC, 19:43:27 local
...
void MarkupTab::on_skip_quotes()
{
    const char *label = "skip-quotes";
    QCheckBox *w = qobject_cast<QCheckBox *>(sender());
    if (w) {
        Bool b = (Bool)w->isChecked();
        setConfigBool( label, b );
...
static CHKBXS chkbxs[] = {
    ...
    { &skip_quotes, "skip-quotes", "Boolean", "markup" },
    ...
    : QWidget(parent)
{
    ...
    skip_quotes = new QCheckBox("skip-quotes (Boolean)");
    if (getConfigBool("skip-quotes")) {
        skip_quotes->setChecked(true);
    }
    connect(skip_quotes,SIGNAL(clicked()),this,SLOT(on_skip_quotes()));
    ...
    MarkupLayout2->addWidget(skip_quotes);
